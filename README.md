...
│
├── polling/
│   └── prPoller.ts                # Fallback-поллинг PR раз в 60 сек (если webhook не доходят)
│
└── ai/
...
- **Невалидный JSON от AI**: сначала делается попытка восстановить через повторный вызов Claude CLI (`repairJson`), если не помогает — постится COMMENT
- **Язык ревью**: совпадает с языком заголовка и описания PR
- **Fallback-поллинг**: если webhook'и не доходят, сервер сам проверяет открытые PR раз в 60 сек и ревьюит те, у которых ещё нет бот-ревью на текущий коммит
